...
namespace ft {
	...
	class Vector {
		...
		/*
		=========================================================================
		Constructors, destructor, operator=, assign
		=========================================================================
		*/
		...
		*/
			...
			void insert(iterator pos, InputIt first, InputIt last, 
				...
				if (empty()) {
					...
					for (size_type i = 0; i < count; ++i) {
						_alloc.construct(_array + insertIdx++, *first++);
						_size++;
					}
				}
			}
		...
		*/
		...
		=========================================================================
		*/
			friend bool operator==(const Vector<value_type, allocator_type>& lhs, const Vector<value_type, allocator_type>& rhs) {
				size_type i = 0;
				size_type j = 0;
			...
			friend bool operator<=(const Vector<value_type, allocator_type>& lhs, const Vector<value_type, allocator_type>& rhs) {
				...
				return (i == lhs.size() && i == rhs.size()) || (i != rhs.size() && (i == lhs.size() || lhs._array[i] < rhs._array[i]));
			}
			...
			friend bool operator>=(const Vector<value_type, allocator_type>& lhs, const Vector<value_type, allocator_type>& rhs) {
				...
				return (i == rhs.size() && i == lhs.size()) || (i != lhs.size() && (i == rhs.size() || rhs._array[i] < lhs._array[i]));
			}

			friend void swap(const Vector<value_type, allocator_type>& lhs, const Vector<value_type, allocator_type>& rhs) {
				lhs.swap(rhs);
			}
